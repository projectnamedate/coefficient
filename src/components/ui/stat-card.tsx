...
export function StatCard({ label, value, subtext, index = 0 }: StatCardProps) {
  ...
  return (
    <div className="group relative gradient-border border border-white/[0.06] bg-white/[0.03] rounded-lg p-4 hover:bg-white/[0.05] hover:scale-[1.01] transition-all duration-300 cursor-default">
      ...
      <div className="relative">
        <p className="text-[11px] font-mono text-lavender/50 uppercase tracking-wider">
          {label}
        </p>
        <p className="text-3xl font-bold text-white mt-1.5 tabular-nums tracking-tight">
          {numericPart !== null ? (
            <AnimatedCounter
              value={numericPart}
              suffix={suffixPart}
